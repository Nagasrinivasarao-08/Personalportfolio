...
const SkillCard = ({ category, skills }: { category: string; skills: string[] }) => (
  ...
  >
    <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-red-500/0 to-red-500/0 group-hover:from-red-500/5 group-hover:to-red-500/5 transition-all duration-500 pointer-events-none" />
      ...
      <div className="flex flex-wrap gap-2">
        {skills.map((skill) => (
          <motion.button
            ...
            initial="initial"
            whileHover="hover"
            className="px-3 py-1.5 text-xs font-medium text-black bg-black/5 hover:bg-red-500/10 border border-black/5 hover:border-red-500/20 rounded-full transition-all duration-300 cursor-default select-none hover:text-[#ff2a2a]"
          >
            {skill}
          </motion.button>
        ))}
  ...
  return (
    <section id="skills" className="relative w-full bg-white py-16 md:py-20 overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 md:px-8">
        ...
        >
          {skillsData.map((cat) => (
            <SkillCard key={cat.name} category={cat.name} skills={cat.items} />
          ))}
        </motion.div>
      </div>
